...
export async function MusicaExperience() {
  const [user, role, latestTracks, trendingWeek, highlightedTracks, upcomingAgenda] = await Promise.all([
    ...
    getTrending({ period: 'week' })
      .then((e) => e.slice(0, TRENDING_HOME_LIMIT))
  ...
  const continueListening = user ? await getContinueListening(user.id).catch(() => []) : []
      ...
      <div className="min-h-screen bg-gate-bg">
        ...
        <main className="md:ml-16 md:pt-20 px-4 sm:px-8 py-8 sm:py-12 flex flex-col items-center">
          ...
          <p className="mt-2 max-w-md text-sm text-gate-blue text-center">
            Veja o que a comunidade está postando, ou use o ícone de música na barra lateral para explorar por gênero.
          </p>
          ...
          {latestTracks.length > 0 && (
            <section className="mt-8 w-full max-w-3xl">
              <h2 className="text-sm font-bold uppercase tracking-widest text-gate-blue">Músicas mais recentes</h2>
          ...
          <Link href="/musicas-recentes" className="mt-6 inline-block rounded-lg border border-gate-azure px-5 py-2.5 text-sm font-semibold text-white transition hover:border-gate-pink hover:text-gate-pink">
  ...
  return (
    <div className="min-h-screen bg-gate-bg">
      ...
      <main className="pt-14 md:pt-20 px-4 sm:px-8 py-8 sm:py-12 flex flex-col items-center">
        <h1 className="text-2xl font-bold text-white text-center">Últimos lançamentos</h1>
        <p className="mt-2 max-w-md text-sm text-gate-blue text-center">Explore a produção musical da comunidade. Faça login para baixar, curtir e comentar.</p>
        <AgendaHomeSection events={upcomingAgenda} />
...
function ContinueListeningSection({ entries, canDownload }: { entries: ContinueListeningEntry[]; canDownload: boolean }) {
  if (entries.length === 0) return null
  const queue = entries.map((e) => toPlayerTrack(e.track))
  return (
    <section className="mt-8 w-full max-w-3xl">
      <h2 className="text-sm font-bold uppercase tracking-widest text-gate-blue">Continue ouvindo</h2>
      <div className="mt-3 divide-y divide-gate-azure/30 rounded-xl border border-gate-azure overflow-hidden">
        {entries.map((entry) => (
          <TrackCard key={entry.track.id} track={entry.track} canDownload={canDownload} isLoggedIn queue={queue} />
        ))}
      </div>
    </section>
  )
...
function HighlightedSection({ tracks, canDownload, isLoggedIn }: { tracks: TrackPublic[]; canDownload: boolean; isLoggedIn: boolean }) {
  if (tracks.length === 0) return null
  const queue = tracks.map(toPlayerTrack)
  return (
    <section className="mt-8 w-full max-w-3xl">
      <h2 className="text-sm font-bold uppercase tracking-widest text-gate-blue">⭐ Em destaque</h2>
      <div className="mt-3 divide-y divide-gate-azure/30 rounded-xl border border-gate-azure overflow-hidden">
        {tracks.map((track) => (
          <TrackCard key={track.id} track={track} canDownload={canDownload} isLoggedIn={isLoggedIn} queue={queue} isHighlighted />
        ))}
      </div>
    </section>
  )
  ...
  return (
    <section className="mt-8 w-full max-w-3xl">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold uppercase tracking-widest text-gate-blue">📈 Em Alta esta semana</h2>
      ...
      <div className="mt-3 divide-y divide-gate-azure/30 rounded-xl border border-gate-azure overflow-hidden">
        {entries.map((entry) => (
          <div key={entry.track.id} className="flex items-start">
            <span className={`w-9 shrink-0 pt-6 text-right text-lg font-bold tabular-nums ${entry.position <= 3 ? 'text-gate-pink' : 'text-gate-blue'}`} aria-label={`Posição ${entry.position}`}>
              {entry.position}
            </span>
...
function AgendaHomeSection({ events }: { events: ArtistEventPublic[] }) {
  ...
  return (
    <section className="mt-8 w-full max-w-3xl">
      ...
      <div className="mt-3 divide-y divide-gate-azure/30 rounded-xl border border-gate-azure overflow-hidden">
        {events.map((event) => (
          <Link key={event.id} href={`/artista/${event.artist.slug}`} className="flex items-center justify-between gap-3 px-4 py-3 transition hover:bg-white/5">
            <div className="min-w-0">
              <p className="text-sm font-medium text-white truncate">{event.title}</p>
              <p className="text-xs text-gate-blue">
                {event.artist.name} · {new Date(event.startsAt).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}
